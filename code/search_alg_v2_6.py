...
def calculate_scores(data, evi_set, model, device, max_seq_len, tokenizer, THRED=32, pos_ids=False):
    def helper_fn(sub_evi_set):
        ...
        _, input_ids, input_masks, segment_ids, position_ids = examples_to_tensors(examples, tokenizer, max_seq_len, pos_ids)
...
def predict_N_or_TF(evi_set, label2id, label_map, scores, top_k=1):
    ...
    if torch.prod(TF <= N):
        ...
        _, max_inds = TF[indics].view(-1).topk(k=top_k, largest=True)
        for ind in max_inds:
            x, y = ind // 2, ind % 2
            labels.append(y + 1)
...
def predict_T_or_F(evi_set, label2id, label_map, scores, top_k=1):
    ...
    _, N_min_indics = N.view(-1).topk(k=top_k, largest=False)
    _, TF_max_indics = TF[N_min_indics].view(-1).topk(k=top_k, largest=True)
    evidences, labels, max_scores = [], [], []
    for ind in TF_max_indics:
        x, y = ind // len(pos), ind % len(pos)
        evidences.append(evi_set[N_min_indics[x]])
        labels.append(y + 1)
        max_scores.append(TF[N_min_indics[x], y].item())
    
    return evidences, labels, max_scores


def greedy_search(data, model, tokenizer, label2id, device, label_map,
                  max_seq_len=162, max_sent=5, THRED=32, pos_ids=False):
    '''
    label_map: ['NOT ENOGH INFO', 'SUPPORTS', 'REFUTES']
    '''
    pool = [[doc_id, sent_id] \
             for doc_id in data['doc2sents'] \
               for sent_id in data['doc2sents'][doc_id] \
                if len(data['doc2sents'][doc_id][sent_id]) > 0]
    evidence, label = [], None
    ...
    with torch.no_grad():
        for _ in range(max_sent):
            ...
            scores = calculate_scores(data, evi_set, model, device, max_seq_len, tokenizer,
                                      THRED=THRED, pos_ids=pos_ids)
