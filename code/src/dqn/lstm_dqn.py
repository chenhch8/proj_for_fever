...
from models import AttentionLayer, ScoreLayer, AutoBertModel 
...
class QNetwork(nn.Module):
    def __init__(self, args, dropout=0.1, input_size=768, hidden_size=768):
    ...
    def forward(self, states, state_mask, actions, actions_mask):
        '''
        states: [batch, seq, input_size]
        state_mask: [batch, seq]
        actions: [batch, seq2, input_size]
        actions_mask: [batch, seq2]

        return:
            [batch * available_seq2, num_labels]
        '''
        batch, seq, input_size = states.size()
...
class LstmDQN(BaseDQN):
    def __init__(self, args):
        ...
        self.q_net = QNetwork(
            args,
            input_size=5 * config.hidden_size,
