...
def convert_tokens_to_bert_inputs(all_tokens_a: List[long],
                                  all_tokens_b: List[long],
                                  max_seq_len: int, 
                                  CLS: long, SEP: long, device=None) -> dict:
    ...
    return {
        'inputs_ids': torch.tensor(all_inputs_ids, dtype=torch.long, device=device),
...
class BertDQN(BaseDQN):
    ...
    def token(self, text_sequence: str) -> List[long]:
    ...
    def convert_to_inputs_for_select_action(self, state: State, actions: List[Action]) -> List[dict]:
        condidate = reduce(lambda seq1, seq1: seq1 + seq2,
                           map(lambda sent: sent.tokens, state.candidate))
        length = self.max_seq_length - 3 - len(state.claim.tokens) - len(condiate)
        assert length > 0
    ...
    def convert_to_inputs_for_update(self, states: List[State], actions: List[Action]) -> dict:
        ...
        for state, action in enumerate(states, actions):
            tokens_a = state.claim.tokens
            condidate = reduce(lambda seq1, seq1: seq1 + seq2,
                               map(lambda sent: sent.tokens, state.candidate))
            tokens_b = condidate + actions[i].sentence.tokens
