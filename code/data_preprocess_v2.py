class CONFIG:
    pass
config = CONFIG()
config.train = 'data/nn_doc_retr_1_train.jsonl'
config.dev = 'data/nn_doc_retr_1_shared_task_dev.jsonl'
# config.test = '../data/fever/shared_task_test.jsonl'
config.db = 'data/fever-coref.db'
config.max_evidence_length = 5
config.to_train = 'data/train_process(%d)-v3.jsonl' % config.max_evidence_length
config.to_dev = 'data/dev_process(%d)-v3.jsonl' % config.max_evidence_length
            ...
            for line in tqdm(f.readlines()):
                ...
                for evidence in instance['evidence']:
                    ...
                    for sent in evidence:
                        ...
                        new_evidence.append([sent[2], sent[3]])
                    if len(new_evidence) > 0 and new_evidence not in process_evidence:
                        process_evidence.append(new_evidence)
...
def evidence_construct(doc2sents, evidence_set=None, max_evidence_length=10):
    result = []
    pool = [[doc_id, sent['line_num']] for doc_id, sents in doc2sents.items() \
                                        for sent in sents if len(sent['sentences']) > 0]
    if len(evidence_set) == 0:
        result.append([
            None, {1: [[None, [[sent] for sent in pool]]]}, None])
    else:
        subset_cache = []  # 记录每个证据的所有子集
        for evidence in evidence_set:
#             if len(evidence) > max_evidence_length:  # 去除过长的 evidence
#                 continue                             # 防止后面的combinations数量过多造成程序卡死
            ...
            for i in range(1, len(evidence) + 1):
                # 遍历每个长度为 i 的证据子集
                for subset in combinations(evidence, i):
                    subset = list(subset)
                    subset_cache.append(subset)
                    fake_subset_list = []
                    # 构造“差一”负子集
                    for j in range(len(subset)):
                        for sent in pool:
                            if sent in subset: continue
                            fake_subset = deepcopy(subset)
                            fake_subset[j] = sent
                            if fake_subset in evidence: continue  # 去除负样本为证据
            ...
            for sent in pool:
                ...
                supset = evidence + [sent]
                if supset in evidence_set: continue  # 超集为证据
            ...
            result.append([evidence, dict(all_subset_list), all_supset_list])
        # 去除负子集为其它证据的子集
        for _, all_subset_list, _ in result:
            for k in all_subset_list:
                subset_list = all_subset_list[k]
                for subset in subset_list:
                    subset[1] = list(filter(lambda x: x not in subset_cache, subset[1]))
                    if len(subset[1]) == 0:
                        pprint(doc2sents)
                        pprint(pool)
                        pprint(evidence_set)
                        pprint(subset_cache)
                        assert len(subset[1]) > 0
    if len(result) == 0:
        pprint(doc2sents)
        pprint(pool)
        pprint(evidence_set)
...
def sents_id2sents_str(doc2sents, evidences):
    ...
    for evidence, all_subset_list, all_supset_list in evidences:
...
fever_db = FeverDataBase(config.db)
...
#     print(query_ids)
    ...
    evidences = evidence_construct(doc2sents, data['process_evidence'],
                                   max_evidence_length=config.max_evidence_length)
    assert len(evidences) > 0
    sents = sents_id2sents_str(doc2sents, evidences)
...
with open(config.to_train, 'w') as f:
    print('Saving to %s ...' % config.to_train)
...
with open(config.to_dev, 'w') as f:
    print('Saving to %s ...' % config.to_dev)
