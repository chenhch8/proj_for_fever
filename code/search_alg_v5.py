...
def predict_N_or_TF(evi_set, label2id, label_map, scores, top_k=1):
    ...
    if torch.prod(TF <= N):
        ...
        for ind in max_inds:
            ...
            ids.append(indics[x])
    return evidences, labels, max_scores, ids
...
def greedy_search(data, model, tokenizer, label2id, device, label_map, max_seq_len=162, max_sent=5, THRED=32):
    ...
    score_per_class = torch.ones((len(label_map)), dtype=torch.float).to(device)
    with torch.no_grad():
        for _ in range(max_sent):
            ...
            cur_evidences, cur_labels, cur_max_scores, ids = predict_T_or_F(evi_set,
                                                                       label2id,
                                                                       label_map,
                                                                       scores,
                                                                       top_k=1)
            ...
            score_per_class.mul_(scores[ids[0]])
            del scores
    label = score_per_class.argmax().item()
    if label == label2id[label_map[0]]:
        evidence = []
